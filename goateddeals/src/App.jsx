...
export default function App() {
  ...
  return (
    <div>
        ...
        <div style={{ flex:1, maxWidth:"560px", margin:"0 auto", position:"relative" }}>
          ...
          {search && (
            <button onClick={()=>setSearch("")}
              style={{ position:"absolute", right:"10px", top:"50%", transform:"translateY(-50%)", background:"rgba(245,79,30,0.15)", border:"none", borderRadius:"50%", width:"22px", height:"22px", color:"#F54F1E", fontSize:"13px", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>×</button>
          )}
        </div>
      ...
      <div style={{ background:"rgba(245,166,35,0.07)", borderBottom:"1px solid rgba(245,166,35,0.18)", padding:"9px 24px" }}>
        <div style={{ maxWidth:"1240px", margin:"0 auto", display:"flex", alignItems:"center", gap:"10px", flexWrap:"wrap", justifyContent:"center" }}>
          <span style={{ fontSize:"14px" }}>📢</span>
          <p style={{ fontSize:"12px", color:"#C8A84B", lineHeight:1.5, textAlign:"center" }}>
            <strong style={{ color:"#F5A623" }}>Affiliate Disclosure:</strong> As an Amazon Associate, GoatedDeals earns from qualifying purchases. We also participate in the Flipkart Affiliate Programme. Clicking product links and buying may earn us a commission — <strong style={{ color:"#F5A623" }}>at no extra cost to you.</strong>
          </p>
        </div>
      </div>
          ...
          <div className="trust-g" style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(230px,1fr))", gap:"16px" }}>
            {[
              ...
              <div key={b.t} className={`tcard reveal reveal-delay-${b.delay+1}`}>
                ...
                <div style={{ color:"#5E5E78", fontSize:"13px", lineHeight:1.65 }}>{b.d}</div>
              </div>
            ))}
          </div>
        </section>
      ...
      <footer style={{ borderTop:"1px solid #12121C", padding:"44px 24px 32px", textAlign:"center", color:"#5E5E78", fontSize:"13px", lineHeight:1.9, background:"#08080F" }}>
        <div style={{ marginBottom:"16px" }}>
          <span style={{ fontFamily:"'Rajdhani',sans-serif", fontSize:"24px", fontWeight:700, color:"#E8E8F0" }}>GOATED<span style={{ color:"#F54F1E" }}>DEALS</span></span>
        </div>

        {/* Primary mandatory disclosure box */}
        <div style={{ maxWidth:"680px", margin:"0 auto 20px", background:"rgba(245,166,35,0.06)", border:"1px solid rgba(245,166,35,0.2)", borderRadius:"12px", padding:"16px 20px" }}>
          <p style={{ fontSize:"13px", color:"#C8A84B", lineHeight:1.7 }}>
            <strong style={{ color:"#F5A623", display:"block", marginBottom:"4px" }}>📢 Affiliate Disclosure</strong>
      ...
      <div className="mobile-cta">
        <button onClick={()=>window.scrollTo({top:400,behavior:"smooth"})} style={{ pointerEvents:"all", width:"100%", background:"linear-gradient(135deg,#F54F1E,#F5A623)", color:"white", border:"none", borderRadius:"14px", padding:"15px", fontSize:"16px", fontWeight:700, cursor:"pointer", boxShadow:"0 8px 32px rgba(245,79,30,0.45)" }}>
          🛍 View Best Deals
        </button>
      </div>
      ...
      <button onClick={()=>isAdmin?setShowPanel(true):setShowLogin(true)} title="Admin"
        onMouseEnter={e=>{ e.currentTarget.style.borderColor="#F54F1E"; e.currentTarget.style.color="#F54F1E"; e.currentTarget.style.boxShadow="0 0 0 4px rgba(245,79,30,0.15)"; }}
        onMouseLeave={e=>{ e.currentTarget.style.borderColor="#2A2A3A"; e.currentTarget.style.color="#5E5E78"; e.currentTarget.style.boxShadow="0 4px 24px rgba(0,0,0,0.5)"; }}
        style={{ position:"fixed", bottom:"24px", right:"24px", width:"52px", height:"52px", borderRadius:"50%", background:"#12121C", border:"1.5px solid #2A2A3A", color:"#5E5E78", fontSize:"22px", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", transition:"all 0.2s", zIndex:500, boxShadow:"0 4px 24px rgba(0,0,0,0.5)" }}>⚙</button>
          ...
          <ModalCard>
            ...
            <div className="mgrid" style={{ display:"flex" }}>
              <div className="mleft" style={{ flex:"0 0 50%", padding:"26px", borderRight:"1px solid #1C1C2A" }}>
                <div style={{ borderRadius:"16px", overflow:"hidden", background:"#08080F", marginBottom:"14px", aspectRatio:"1" }}>
                  ...
                  <div style={{ display:"flex", gap:"9px", flexWrap:"wrap" }}>
                    {allImgs.map((img,i)=>(
                      <img key={i} src={img} alt="" loading="lazy" onClick={()=>setImgIdx(i)}
                        style={{ width:"60px", height:"60px", objectFit:"cover", borderRadius:"10px", cursor:"pointer", border:`2px solid ${i===imgIdx?"#F54F1E":"#1C1C2A"}`, transition:"border-color 0.2s, transform 0.2s", transform:i===imgIdx?"scale(1.05)":"scale(1)" }}
              ...
              <div style={{ flex:1, padding:"26px", minWidth:0 }}>
                ...
                <div style={{ marginBottom:"16px" }}>
                  <div style={{ fontSize:"38px", fontWeight:800, color:"#F54F1E", lineHeight:1 }}>{selected.price}</div>
                  <div style={{ display:"flex", alignItems:"center", gap:"8px", marginTop:"4px" }}>
                    {selected.originalPrice && <span style={{ fontSize:"15px", color:"#5E5E78", textDecoration:"line-through" }}>{selected.originalPrice}</span>}
                ...
                <p style={{ fontSize:"14px", color:"#8888A8", lineHeight:1.75, marginBottom:"18px" }}>{selected.description}</p>
                <div style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"22px", fontSize:"13px", background:"rgba(34,197,94,0.07)", border:"1px solid rgba(34,197,94,0.2)", borderRadius:"8px", padding:"8px 12px" }}>
                  ...
                  <span style={{ color:"#6E6E88" }}>Quality verified · Lowest price tracked · {selected.reviews?.toLocaleString()} buyers</span>
                </div>
                ...
                <div style={{ marginTop:"12px", background:"rgba(245,166,35,0.06)", border:"1px solid rgba(245,166,35,0.15)", borderRadius:"8px", padding:"8px 12px", display:"flex", alignItems:"flex-start", gap:"7px" }}>
                  <span style={{ fontSize:"12px", flexShrink:0, marginTop:"1px" }}>📢</span>
                  <p style={{ fontSize:"11px", color:"#A89060", lineHeight:1.55 }}>
                    <strong style={{ color:"#C8A84B" }}>Affiliate link</strong> — As an Amazon Associate / Flipkart Affiliate, we earn from qualifying purchases at no extra cost to you. Price may vary on store.
                  </p>
                </div>
              </div>
            </div>
        ...
        <Overlay onClose={()=>{ setShowLogin(false); setLoginErr(""); }}>
          <ModalCard maxWidth="370px" style={{ padding:"38px" }}>
            <div style={{ textAlign:"center", marginBottom:"28px" }}>
              <div style={{ width:"56px", height:"56px", background:"linear-gradient(135deg,#F54F1E,#F5A623)", borderRadius:"16px", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"28px", margin:"0 auto 16px" }}>🐐</div>
              <h3 style={{ fontFamily:"'Rajdhani',sans-serif", fontSize:"26px", fontWeight:700 }}>Admin Access</h3>
              <p style={{ fontSize:"13px", color:"#5E5E78", marginTop:"6px" }}>Enter your credentials to manage deals</p>
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:"12px" }}>
              ...
              <button onClick={handleLogin} style={{ background:"linear-gradient(135deg,#F54F1E,#F07052)", color:"white", border:"none", borderRadius:"10px", padding:"14px", fontSize:"15px", fontWeight:700, cursor:"pointer", marginTop:"4px", transition:"transform 0.2s" }}
          ...
          <ModalCard maxWidth="660px">
            ...
            <div style={{ padding:"24px 28px 0", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
                <div style={{ width:"38px", height:"38px", background:"linear-gradient(135deg,#F54F1E,#F5A623)", borderRadius:"10px", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"20px" }}>⚙️</div>
                <h3 style={{ fontFamily:"'Rajdhani',sans-serif", fontSize:"24px", fontWeight:700 }}>Admin Panel</h3>
              </div>
              <button onClick={()=>setShowPanel(false)} style={{ background:"none", border:"none", color:"#5E5E78", fontSize:"24px", cursor:"pointer" }}>×</button>
            </div>
            ...
            <div style={{ padding:"18px 28px" }}>
              <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"10px" }}>
                {[
                  ...
                  <div key={s.label} style={{ background:"#08080F", borderRadius:"12px", padding:"14px", textAlign:"center", border:"1px solid #1C1C2A" }}>
            ...
            <div style={{ padding:"0 28px", display:"flex", gap:"8px", marginBottom:"18px" }}>
              {[{k:"add",l:"➕ Add Product"},{k:"manage",l:`📋 Manage (${allManaged.length})`}].map(t=>(
                <button key={t.k} onClick={()=>setAdminTab(t.k)}
                  style={{ padding:"9px 20px", borderRadius:"10px", border:`1.5px solid ${adminTab===t.k?"#F54F1E":"#1C1C2A"}`, background:adminTab===t.k?"rgba(245,79,30,0.1)":"transparent", color:adminTab===t.k?"#F54F1E":"#5E5E78", fontSize:"13px", fontWeight:600, cursor:"pointer", transition:"all 0.18s" }}>{t.l}</button>
              ))}
            </div>
            ...
            {adminTab==="add" && (
              <div style={{ padding:"0 28px 28px" }}>
                <div style={{ background:"#08080F", borderRadius:"16px", padding:"22px", border:"1px solid #1C1C2A" }}>
                  <h4 style={{ color:"#F5A623", fontSize:"15px", fontWeight:700, marginBottom:"18px" }}>📦 Post New Product</h4>
                  <div style={{ display:"flex", flexDirection:"column", gap:"14px" }}>
                    ...
                    {formMsg.text && (
                      <div style={{ background:formMsg.type==="success"?"rgba(34,197,94,0.08)":"rgba(245,79,30,0.08)", border:`1px solid ${formMsg.type==="success"?"rgba(34,197,94,0.25)":"rgba(245,79,30,0.25)"}`, borderRadius:"10px", padding:"12px 14px", fontSize:"13px", color:formMsg.type==="success"?"#22C55E":"#F54F1E" }}>
                        {formMsg.text}
                      </div>
                    )}
                    ...
                    <button onClick={handleAdd}
                      ...
                      style={{ background:"linear-gradient(135deg,#F54F1E,#F5A623)", color:"white", border:"none", borderRadius:"12px", padding:"15px", fontSize:"15px", fontWeight:700, cursor:"pointer", transition:"transform 0.2s, box-shadow 0.2s", boxShadow:"0 6px 24px rgba(245,79,30,0.35)" }}>
            ...
            {adminTab==="manage" && (
              <div style={{ padding:"0 28px 28px" }}>
                ...
                {hiddenDemoIds.length > 0 && (
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:"rgba(245,166,35,0.08)", border:"1px solid rgba(245,166,35,0.25)", borderRadius:"12px", padding:"12px 16px", marginBottom:"14px" }}>
                    <span style={{ fontSize:"13px", color:"#F5A623" }}>⚠️ {hiddenDemoIds.length} built-in product{hiddenDemoIds.length>1?"s":""} hidden</span>
                    <button onClick={handleRestoreAll}
                      style={{ background:"rgba(245,166,35,0.15)", border:"1px solid rgba(245,166,35,0.4)", borderRadius:"8px", color:"#F5A623", fontSize:"12px", fontWeight:700, padding:"6px 14px", cursor:"pointer" }}>
                      ↩ Restore All
                    </button>
                  </div>
                )}
                ...
                {allManaged.length === 0 ? (
                  <div style={{ background:"#08080F", borderRadius:"16px", padding:"40px", textAlign:"center", border:"1px solid #1C1C2A" }}>
                    ...
                    <p style={{ color:"#5E5E78", fontSize:"14px" }}>No products yet.</p>
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:"1px", background:"#1C1C2A", borderRadius:"16px", overflow:"hidden", border:"1px solid #1C1C2A" }}>
                    {allManaged.map((p, i) => {
                      ...
                      return (
                        <div key={p.id}
                          style={{ display:"flex", alignItems:"center", gap:"12px", background: isHidden?"#0D0D16":"#12121C", padding:"14px 16px", borderBottom:i<allManaged.length-1?"1px solid #1C1C2A":"none", transition:"background 0.18s", opacity: isHidden ? 0.45 : 1 }}
                          onMouseEnter={e=>{ if(!isHidden) e.currentTarget.style.background="#181824"; }}
                          onMouseLeave={e=>{ e.currentTarget.style.background=isHidden?"#0D0D16":"#12121C"; }}>
                          ...
                          <div style={{ flex:1, minWidth:0 }}>
                            <div style={{ display:"flex", alignItems:"center", gap:"6px", marginBottom:"3px" }}>
                              ...
                              {isHidden && (
                                <span style={{ fontSize:"9px", fontWeight:700, background:"rgba(245,166,35,0.12)", color:"#F5A623", border:"1px solid rgba(245,166,35,0.25)", borderRadius:"4px", padding:"1px 6px", flexShrink:0 }}>HIDDEN</span>
                              )}
                            </div>
                          ...
                          <div style={{ display:"flex", gap:"7px", flexShrink:0 }}>
                            <a href={p.storeUrl} target="_blank" rel="noopener noreferrer"
                              style={{ background:"rgba(245,166,35,0.1)", border:"1px solid rgba(245,166,35,0.25)", borderRadius:"8px", color:"#F5A623", fontSize:"12px", fontWeight:600, padding:"6px 11px", cursor:"pointer", textDecoration:"none" }} title="Open store">🔗</a>
                            ...
                            {isHidden ? (
                              ...
                              <button onClick={()=>handleDelete(p.id)}
                                ...
                                style={{ background:"rgba(245,79,30,0.08)", border:"1px solid rgba(245,79,30,0.25)", borderRadius:"8px", color:"#F54F1E", fontSize:"12px", fontWeight:600, padding:"6px 11px", cursor:"pointer", transition:"all 0.18s" }}>🗑 Delete</button>
                            )}
                          </div>
                        </div>
                      );
